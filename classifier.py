...
class Classifier(object):
    ...
    def train(self, dataset='wikiart', val_size=1000, batch_size=64, epochs=25):
        ...
        val = [
          get_image(val_file,
                    ...
                    crop=False,
                    grayscale=False) for val_file in val_files]
        ...
        for epoch in xrange(epochs):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if np.mod(counter, 250) == 1:
                    loss, _, summary_str, acc = self.sess.run([self.loss, self.update, self.summary, self.accuracy], feed_dict={
                    self.inputs: val,
                    self.y: val_y,
                })
                    self.writer.add_summary(summary_str, counter)
                    print("VALIDATION: [%2d] [%4d/%4d] time: %4.4f, val_loss: %.8f, val_acc: %.8f" \
                        % (epoch, idx, batch_idxs,
                        time.time() - start_time, loss, acc))
        self.save(self.checkpoint_dir, counter)
