...
def vanilla_wgan(model, image, y, reuse=False):
...
def vanilla_wgan_no_y(model, image, reuse=False):
...
def wgan_slim(model, image, y, reuse=False):
    with tf.variable_scope("discriminator") as scope:
        if reuse:
            scope.reuse_variables()
        yb = tf.reshape(y, [-1, 1, 1, model.y_dim])
...
def wgan_no_y_slim(model, image, reuse=False):
    with tf.variable_scope("discriminator") as scope:
        if reuse:
            scope.reuse_variables()
        h0 = lrelu(layer_norm(conv2d(image, model.df_dim, k_h=4, k_w=4, name='d_h0_conv',padding='VALID')))
